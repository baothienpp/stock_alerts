...
def get_past_date(df, return_date_cols=True):
    df['last_year'] = df['datetime'].apply(lambda x: datetime.date(day=1, month=1, year=x.year - 1))
...
def calculate_roc(df, past_date_col='last_day'):
    ...
    lastprice_df = pd.merge_asof(df[['datetime', 'symbol', 'close', past_date_col]],
                                 df[['datetime', 'symbol', 'close']],
    ...
    lastprice_df['roc_' + past_date_col] = lastprice_df.apply(
                                                    ...
                                                    axis=1)
    lastprice_df.drop(columns=[past_date_col, 'datetime_y', 'close_y'])
    return lastprice_df
